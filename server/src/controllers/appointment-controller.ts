...
export class AppointmentController {
  async getAppointmentById(req: AuthRequest, res: Response) {
    try {
      ...
      if (req.user?.role === 'PATIENT' && appointment.appointedPatient !== req.user.id) {
        return res.status(403).json({ error: 'Forbidden: You can only view your own appointments' });
      }
      
      if (req.user?.role === 'DOCTOR' && appointment.appointedDoctor !== req.user.id) {
  ...
  async getAppointmentsByQuery(req: AuthRequest, res: Response) {
    try {
      ...
      if (patientId) {
        if (req.user?.role === 'PATIENT' && req.user.id === patientId) {
          const result = await appointmentService.getAppointmentsByPatient(patientId as string, page, pageSize);
      ...
      if (doctorId) {
        if (req.user?.role === 'DOCTOR' && req.user.id === doctorId) {
          const result = await appointmentService.getAppointmentsByDoctor(doctorId as string, page, pageSize);
          return res.json(result);
        }
      ...
      return res.status(400).json({ error: 'Missing patientId or doctorId parameter' });
    } catch (error: any) {
  ...
  async getAppointments(req: AuthRequest, res: Response) {
    try {
      ...
      if (req.user?.role === 'PATIENT' && req.user.id === id) {
        const result = await appointmentService.getAppointmentsByPatient(id, page, pageSize);
        return res.json(result);
      }

      if (req.user?.role === 'DOCTOR' && req.user.id === id) {
        const result = await appointmentService.getAppointmentsByDoctor(id, page, pageSize);
        return res.json(result);
      }
  ...
  async updateAppointment(req: AuthRequest, res: Response) {
    try {
      ...
      if (req.user?.role !== 'DOCTOR' && req.user?.role !== 'SECRETARY') {
        return res.status(403).json({ error: 'Forbidden: Only doctors and secretaries can update appointments' });
      }
